...
class Events_model extends Base_posts_model {
	...
	public $name = 'events'; // this property is usually just the same as the table name but can be different and is used for image folders and tag and category contexts
	public $img_width = 150; // default image width dimensions
	public $img_height = 100; // default image height dimensions
	...
	{
		$this->db->select('events.id, events.name, fuel_categories.name AS category, start_date, end_date, events.published', FALSE);
	...
	{
		$fields['name'] = array();
		$fields['start_date_fromequal'] = array('type' => 'date', 'label' => 'Start date');
		$fields['end_date_toequal'] = array('type' => 'date', 'label' => 'End date');
		$fields['location'] = array();
		$fields['category'] = array('type' => 'select', 'model' => array(FUEL_FOLDER => 'fuel_categories_model'), 'first_option' => lang('label_select_one'));
		return $fields;
	}
...
class Event_model extends Base_module_record {
	...
	{
		return site_url('events#event-'.$this->id);
	}
